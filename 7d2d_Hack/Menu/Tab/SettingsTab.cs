...
namespace Game7D2D.Menu.Tab
{
    public class SettingsTab : MonoBehaviour
    {
        ...
        public static void Tab()
        {
            ...
            if (Global.Settings.MiscOptions.GlobalColors.Count > 0)
            {
                scrollPosition1 = GUILayout.BeginScrollView(scrollPosition1/*, GUILayout.Width(480)*/);

                List<string> keys = new List<string>(Global.Settings.MiscOptions.GlobalColors.Keys);
                for (int i = 0; i < keys.Count; i++)
                {
                    ...
                    if (SelectedColorIdentifier == key)
                    {
                        ...
                        Global.Settings.MiscOptions.GlobalColors[key] = cc;
